...
r = np.transpose(utils.AU_to_m(system.initial_positions))

def habitable_zone(planets, R, sun_R, T, r):

    '''
    function for calculating flux and total energy received, minimum
    solar panel necessary, surface area and surface temperature of
    each planet in our solar system to determine whether or not
    they are within the habitable zone
    '''

    sigma = 5.670374419*10**(-8)                                    # Stefan-Boltzmann's constant [W/(m**2*K**4)]
    F_sun = sigma*T**4                                              # the flux emitted from our sun [W/m**2]
    ...
    for i in range(len(planets)):
        ...
        F[i] = A*F_sun/(4*np.pi*np.linalg.norm(r[i])**2)            # the received flux per square meter for the current planet [W/m**2]
