...
def generate_picture(theta, phi, width = 640, length = 480, theta0 = np.pi/2, phi0 = 0):
    ...
    '''
    function for decomposing and regenerating reference picture
    '''
...
def generate_picture_360(X, Y, phi_array, width = 640, length = 480, theta0 = np.pi/2):
    ...
    '''
    function for using stereographic projection to generate 
    3600 flattened reference pictures from the spherical 
    360 picture taken by the satellite
    '''
        ...
        for i in range(length):
            for j in range(width):
                ...
                for k in range(3):
                    pixels[i, j, k] = sky_sphere[int(pxl_idx[i, j]), k+2]
        picture = Image.fromarray(pixels)
        picture.save(f'phi_{phi0:.1f}_deg.png')
        gif_array.append(ig.imread(f'phi_{phi0:.1f}_deg.png'))
