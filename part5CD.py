...
from part5AB import fuel_consumption, r_Buttercup, time_stamp, N_H2, fuel_m, t0, phi1, phi2, deltav2, T_transfer, home_pos, max_time, times, exact_planet_positions
...
mission.take_picture(filename='sky_picture_part5.png', full_sky_image_path='himmelkule.npy')
phi0 = find_phi0('sky_picture_part5.png')
...
def main():
    ...
    def orbit_stability(N, dt, x0, y0, vx0, vy0, M, R):
        ...
        '''
        function for simulating the spacecraft's 
        three next orbits to check if it's stable 
        '''
    ...
    plt.plot(xy[:, 0]*1e-3, xy[:, 1]*1e-3, color = 'gold', label = 'spacecraft')
    plt.scatter(0, 0, color = 'orange', label = 'Buttercup')
    ...
    fig = plt.gcf()
    plt.show()
    fig.savefig('spacecraftorbits_3yrs.pdf')

    t = np.linspace(0, len(r)*dt, len(r))*52/yr
    plt.plot(t, vr*1e-3, color = 'slateblue')
    plt.xlabel('t [weeks]')
    ...
    fig = plt.gcf()
    plt.show()
    fig.savefig('radialvelocity_3yrs.pdf')

    plt.plot(t, vtheta*180/np.pi, color = 'mediumvioletred')
    plt.xlabel('t [weeks]')
    ...
    fig = plt.gcf()
    plt.show()
    fig.savefig('angularvelocity_3yrs.pdf')
...
RESULTS:
    ...
    *** Achievement unlocked: No free launch! ***
        ...
        MANUAL ORIENTATION:
    Picture written to sky_picture_part5.png.
    ...
    *** Achievement unlocked: Well-oriented! ***
    ...
    XML file where_is_homeplanet.xml was saved in XMLs/.
    It can be viewed in MCAst.
    Picture saved to where_is_homeplanet.xml.
    ...
    XML file checking_position1.xml was saved in XMLs/.
    It can be viewed in MCAst.
    Picture saved to checking_position1.xml.
    Camera pointing towards direction of motion.
    XML file checking_motion_direction.xml was saved in XMLs/.
    It can be viewed in MCAst.
    Picture saved to checking_motion_direction.xml.
    ...
    Camera pointing towards planet 3.
    XML file checking_position2.xml was saved in XMLs/.
    It can be viewed in MCAst.
    Picture saved to checking_position2.xml.
    ...
    XML file checking_position3.xml was saved in XMLs/.
    It can be viewed in MCAst.
    Picture saved to checking_position3.xml.
    ...
    XML file checking_orbit_video.xml was saved in XMLs/.
    It can be viewed in MCAst.
    Video with 1000 frames saved to checking_orbit_video.xml.
